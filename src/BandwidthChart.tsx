...
const COLOR_IN  = '#34d399'  // download — verde
const COLOR_OUT = '#fb923c'  // upload   — naranja

function formatBw(val: number, unit: string): string {
  const u = unit.toLowerCase()
  // If Zabbix already returns in Mbps/Kbps/etc, just round
...
function timeLabel(clock: number): string {
...
export default function BandwidthChart({ inData, outData, unit, hours }: Props) {
  ...
  const all = [...inData.map(p => p.value), ...outData.map(p => p.value)]
  ...
  function interpolate(data: HistoryPoint[], t: number): number | null {
    ...
    const ratio = (t - a.clock) / (b.clock - a.clock)
  ...
  return (
    <div className="bw-chart-wrap">
      ...
      >
        ...
        {yTicks.map((t, i) => (
          <g key={i}>
            <line x1={PAD.left} y1={t.y} x2={PAD.left + PW} y2={t.y}
              stroke="#1e293b" strokeWidth={i === 0 ? 1.5 : 1} />
        ...
        {/* Areas */}
        {pathInFill  && <path d={pathInFill}  fill={COLOR_IN}  fillOpacity={0.12} />}
        {pathOutFill && <path d={pathOutFill} fill={COLOR_OUT} fillOpacity={0.12} />}

        {/* Lines */}
        {pathInLine  && <path d={pathInLine}  fill="none" stroke={COLOR_IN}  strokeWidth={1.8} strokeLinejoin="round" />}
        {pathOutLine && <path d={pathOutLine} fill="none" stroke={COLOR_OUT} strokeWidth={1.8} strokeLinejoin="round" />}
        ...
        {hover && (
          <>
            ...
            {hover.inVal !== null && (
              <circle cx={hover.x}
                cy={PAD.top + PH - (hover.inVal / maxVal) * PH}
                r={3.5} fill={COLOR_IN} />
            )}
            {hover.outVal !== null && (
              <circle cx={hover.x}
                cy={PAD.top + PH - (hover.outVal / maxVal) * PH}
                r={3.5} fill={COLOR_OUT} />
            )}
          </>
        )}
      </svg>
