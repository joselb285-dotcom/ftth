...
const SP_PORT_H = 15
const SP_GAP  = 8
const SP_X    = LEFT_W + Math.floor((MID_W - SP_W) / 2)
const LEX     = LEFT_W
const REX     = LEFT_W + MID_W
...
function splitterBoxH(sp: Splitter): number { return SP_HDR + (1 + sp.ratio) * SP_PORT_H }
...
function CablePanel({ cables, sc, panelX, panelW, side, totalH }: {
  cables: FiberCable[]; sc: SpliceCard; panelX: number; panelW: number;
  side: 'left' | 'right'; totalH: number;
}) {
  return (
    <>
      <rect x={panelX} y={0} width={panelW} height={totalH} fill={C_PANEL} />
      <rect x={panelX} y={0} width={panelW} height={SEC_H} fill={C_HDR} />
      <text x={panelX + panelW / 2} y={SEC_H / 2} textAnchor="middle" dominantBaseline="central"
      ...
      <rect x={panelX} y={0} width={panelW} height={totalH} fill="none" stroke={C_BORDER} strokeWidth={0.8} />
    </>
  )
}
...
function SpliceDiagram({ sc, svgRef }: { sc: SpliceCard; svgRef?: React.RefObject<SVGSVGElement | null> }) {
  ...
  if (sc.cables.length === 0) return <div className="sheet-state-msg">Sin fibras registradas.</div>
  ...
  splitters.forEach((sp, si) => {
    ...
    sp.outputPortIds.forEach((pid, oi) => {
      portMap[pid] = { x: SP_X + SP_W, y: sy + SP_HDR + (1 + oi) * SP_PORT_H + SP_PORT_H / 2, color: '#34d399' }
    })
  })
  ...
  return (
    <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' as never }}>
      <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} width={W} height={H}
        ...
        <text x={LEFT_W + MID_W / 2} y={SEC_H / 2} textAnchor="middle" dominantBaseline="central" fontSize={6} fill={C_SUB}>
        ...
        {splitters.map((sp, si) => {
          ...
          return (
            <g key={sp.id}>
              <rect x={SP_X} y={sy} width={SP_W} height={SP_HDR} fill="#1e3a5c" stroke="#3b82f6" strokeWidth={0.8} rx={3} />
              <rect x={SP_X} y={sy + SP_HDR} width={SP_W} height={bh - SP_HDR} fill="#0f1e35" stroke="#3b82f6" strokeWidth={0.8} />
              <text x={SP_X + SP_W / 2} y={sy + SP_HDR / 2} textAnchor="middle" dominantBaseline="central"
                fontSize={6} fontWeight="700" fill="#93c5fd">1×{sp.ratio} {sp.name}</text>
              <circle cx={SP_X} cy={inY} r={4} fill="#3b82f6" stroke="#0a1020" strokeWidth={0.6} />
              {sp.outputPortIds.map((pid, oi) => (
                <circle key={pid} cx={SP_X + SP_W} cy={outY(oi)} r={4} fill="#34d399" stroke="#0a1020" strokeWidth={0.6} />
              ))}
            </g>
          )
        })}
        {sc.connections.map(conn => {
          ...
          return (
            <path key={conn.id} d={bezier(lP.x, lP.y, rP.x, rP.y)} fill="none"
              stroke={color} strokeWidth={conn.active ? 2 : 1.2}
              strokeOpacity={conn.active ? 0.85 : 0.4}
              strokeDasharray={conn.active ? undefined : '5 3'} strokeLinecap="round" />
          )
        })}
...
export default function SpliceSheet({ feature, onClose, onTraceClient }: {
  feature: AppFeature
  onClose: () => void
