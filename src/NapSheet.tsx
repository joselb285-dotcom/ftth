  ...
  return (
    <div className={`client-row${expanded ? ' expanded' : ''}`}>
      <button className="client-row-header" onClick={() => setExpanded(v => !v)}>
        <span className="fiber-dot" style={{ background: FIBER_COLORS[client.fiberColor] ?? '#94a3b8' }} />
        ...
        <div className="client-row-right">
          ...
          <svg className={`expand-icon${expanded ? ' open' : ''}`} width="14" height="14" viewBox="0 0 24 24"
        ...
        <div className="client-detail">
          ...
          {info.onuPowerDbm && (
            <div className="detail-row">
              ...
              Potencia: <span className={`pwr-badge ${pwr.cls}`}>{parseFloat(info.onuPowerDbm).toFixed(2)} dBm</span>
            </div>
          )}
          {info.notes && (
...
export default function NapSheet({
  feature, clients, onClose, onTraceClient, onViewSpliceCard, initialSearch = '',
}: {
  feature: AppFeature
  clients: NapClient[]
  onClose: () => void
      ...
      <div className="nap-sheet">
        ...
        {clients.length > 4 && (
          <div className="sheet-search">
            ...
            <input type="search" placeholder="Buscar cliente, dirección, serial..."
              value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        )}
        <div className="sheet-body">
          {clients.length === 0 && <div className="sheet-state-msg">Esta NAP no tiene clientes registrados.</div>}
          {filtered.length === 0 && search && <div className="sheet-state-msg">Sin resultados para "{search}".</div>}
          {filtered.map(client => (
            <ClientRow key={client.fiberId} client={client}
